...
import { Avatar, Checkbox, Chip, Divider, FormControlLabel, FormGroup, TextField } from '@mui/material';
      ...
      >
        <Box sx={style}>
          <Box sx={{
            ...
            <TextField placeholder='Group name' name='name' value={name} onChange={e => setName(e.target.value)} />
            <Button 
          ...
          <Box bt={3} sx={{
            ...
            {userAssign?.map(user => {
              if (user._id !== userState._id) {
                return (
                  <UserItem key={user._id} props={user} />
                )
              }
            })}
          </Box>
